...
export const addParticipantsToCourse = async (courseId: string, participantIds: string[], institutionId: string, token: string, enrollmentData: any = {}) => {
  try {
    const response = await fetch(`https://lms-server-3-wedg.onrender.com/add-participants/${courseId}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
...
export const updateParticipantEnrollment = async (courseId: string, userId: string, enrollmentData: any, institutionId: string, token: string) => {
  try {
    const response = await fetch(`https://lms-server-3-wedg.onrender.com/update-enrollment/${courseId}/${userId}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
...
export const removeParticipantFromCourse = async (
  ...
  try {
    const response = await fetch(
      `https://lms-server-3-wedg.onrender.com/delete/participant/${courseId}/${userId}`,
      {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
...
export const removeMultipleParticipantsFromCourse = async (
  ...
  try {
    ...
    const debugResponse = await fetch(
      `https://lms-server-3-wedg.onrender.com/api/debug/course/${courseId}`,
      {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
    ...
    const response = await fetch(
      `https://lms-server-3-wedg.onrender.com/delete-participants/multiple/${courseId}`,
      {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
...
export const fetchGroupsCourseData = async (courseId: string, institutionId: string, token: string) => {
  try {
    const response = await fetch(
      `https://lms-server-3-wedg.onrender.com/getAll/groups/courses-data/${courseId}`,
      {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
...
export const removeGroupLeader = async (
  ...
  const response = await fetch(
    `https://lms-server-3-wedg.onrender.com/remove-group-leader/${groupId}/${institution}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
