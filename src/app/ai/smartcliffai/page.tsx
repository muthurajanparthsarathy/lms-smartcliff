...
const SmartCliffCompiler: React.FC = () => {
  ...
  const checkOllamaConnection = async () => {
    try {
      const response = await fetch('https://lms-server-3-wedg.onrender.com/api/chat/health');
  ...
  const testOllamaConnection = async () => {
    ...
    try {
      const response = await fetch('https://lms-server-3-wedg.onrender.com/api/chat/simple', {
        method: 'POST',
        headers: {
...
<body>
  ...
  const stopGeneration = async () => {
    if (currentStreamId) {
      try {
        await fetch('https://lms-server-3-wedg.onrender.com/api/chat/stop-generation', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('https://lms-server-3-wedg.onrender.com/api/chat/ollama-stream', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
