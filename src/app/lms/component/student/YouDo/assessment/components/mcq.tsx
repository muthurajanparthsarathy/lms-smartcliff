...
const sendFinalSubmission = async () => {
  ...
  const prefillPreviousAnswers = async (qs: MCQQuestion[], exId: string) => {
    try {
      ...
      for (const q of qs) {
        try {
          const res = await fetch(`https://lms-smartcliff.vercel.app/courses/answers/previous-submission?courseId=${fCId}&exerciseId=${exId}&questionId=${q._id}&category=${fCat}`, { headers: { Authorization: `Bearer ${token}` } });
          if (!res.ok) continue;
          const data = await res.json();
