...
const sendFinalSubmission = async () => {
  ...
  useEffect(()=>{
    const fetch_=async()=>{
      try{
        ...
        const token=localStorage.getItem('smartcliff_token')||localStorage.getItem('token')||'';
        const res=await fetch(`https://lms-server-3-wedg.onrender.com/exercise/${finalId}`,{ method:'GET',headers:{'Authorization':`Bearer ${token}`,'Content-Type':'application/json'} });
  ...
  const prefillPreviousAnswers = async (qs: MCQQuestion[], exId: string) => {
    try {
      ...
      for (const q of qs) {
        try {
          const res = await fetch(`https://lms-server-3-wedg.onrender.com/courses/answers/previous-submission?courseId=${fCId}&exerciseId=${exId}&questionId=${q._id}&category=${fCat}`, { headers: { Authorization: `Bearer ${token}` } });
          if (!res.ok) continue;
          const data = await res.json();
  ...
  const saveCurrentAnswer=async()=>{
    ...
    try{
      ...
      if(!fCId||!exerciseData._id) return;
      const marks=exerciseData.questionConfiguration?.mcqQuestionConfiguration?.marksPerQuestion||q.mcqQuestionScore||10;
      const sub=async(fd:FormData)=>{ await fetch('https://lms-server-3-wedg.onrender.com/courses/answers/submit',{method:'POST',headers:{'Authorization':`Bearer ${token}`},body:fd}); };
