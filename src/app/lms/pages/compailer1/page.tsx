...
export default function CodeCompiler() {
  ...
  const saveCode = async () => {
    ...
    try {
      const response = await fetch("https://lms-server-3-wedg.onrender.com/save/compiler", {
        method: "POST",
        headers: {
  ...
  const loadUserCode = async () => {
    ...
    try {
      const response = await fetch(`https://lms-server-3-wedg.onrender.com/get/compiler/${userId}/${courseId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("smartcliff_token") || ""}`,
          "Content-Type": "application/json",
        },
