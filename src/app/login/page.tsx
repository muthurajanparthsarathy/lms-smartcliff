...
const SmartCliffLogin = () => {
  ...
  const loginMutation = useMutation({
    mutationFn: async (credentials: { email: string; password: string }) => {
      ...
      const response = await fetch("https://lms-server-3-wedg.onrender.com/user/login", {
        method: "POST",
    ...
    onSuccess: async (data: LoginResponse) => {
      try {
        ...
        const verifyResponse = await fetch("https://lms-server-3-wedg.onrender.com/user/verify-token", {
          method: "POST",
